...
void Tileset::setFirstgid(unsigned int firstgid) {
	firstgid_ = firstgid;
}
...
void Tileset::setTile(unsigned int id, Tile tile) {
	tiles_[id] = tile;
}
...
void Tileset::display(std::ostream & out) const {
...
std::ostream & operator<<(std::ostream &out, const Tileset & tileset) {
	tileset.display(out);
	return out;
}